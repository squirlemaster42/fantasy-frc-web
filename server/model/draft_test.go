...
func TestGetDraftsForUser(t *testing.T) {
	...
	draftId, err := CreateDraft(db, &d)
	assert.NoError(t, err)

	// Invite all other users to the draft
	userTwoInvite, err := InvitePlayer(db, draftId, userOne, userTwo)
	assert.NoError(t, err)
	userThreeInvite, err := InvitePlayer(db, draftId, userOne, userThree)
	assert.NoError(t, err)
	userFourInvite, err := InvitePlayer(db, draftId, userOne, userFour)
	assert.NoError(t, err)
	userFiveInvite, err := InvitePlayer(db, draftId, userOne, userFive)
	assert.NoError(t, err)
	userSixInvite, err := InvitePlayer(db, draftId, userOne, userSix)
	assert.NoError(t, err)
	_, err = InvitePlayer(db, draftId, userOne, userSeven)
	assert.NoError(t, err)
	_, err = InvitePlayer(db, draftId, userOne, userEight)
	assert.NoError(t, err)
	...
	drafts, err := GetDraftsForUser(db, userOne)
	assert.NoError(t, err)
...
func TestGetPicksInDraft(t *testing.T) {
	...
	userSixInvite, err := InvitePlayer(db, draftId, userOne, userSix)
	assert.NoError(t, err)
	userSevenInvite, err := InvitePlayer(db, draftId, userOne, userSeven)
	assert.NoError(t, err)
	userEightInvite, err := InvitePlayer(db, draftId, userOne, userEight)
	assert.NoError(t, err)
