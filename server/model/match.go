...
func AddMatch(context context.Context, database *sql.DB, tbaId string) {
	...
	a.NoError(context, err, "Failed to prepare query")
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "AddMatch: Failed to close statement", "error", err)
		}
	}()
	_, err = stmt.Exec(tbaId, false, 0, 0)
	a.NoError(context, err, "Failed to insert into database")
}

func UpdateScore(context context.Context, database *sql.DB, tbaId string, redScore int, blueScore int) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UpdateScore: Failed to close statement", "error", err)
		}
	}()
	_, err = stmt.Exec(true, redScore, blueScore, tbaId)
	a.NoError(context, err, "Failed to update in database")
}

// All validity checks should be done before now, so we can have this many asserts here
func GetMatch(context context.Context, database *sql.DB, tbaId string) *Match {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "GetMatch: Failed to close statement", "error", err)
		}
	}()
	match := Match{}
