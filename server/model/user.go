...
func RegisterUser(context context.Context, database *sql.DB, username string, password string) uuid.UUID {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "RegisterUser: Failed to close statement", "error", err)
		}
	}()
	userUuid := uuid.New()
	assert.NoError(context, err, "Failed to create uuid")
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(password), 14)
	assert.NoError(context, err, "Failed to generate password hash")
	err = stmt.QueryRow(userUuid, username, string(hashedPassword)).Scan(&userUuid)
	assert.NoError(context, err, "Failed to register user")
	return userUuid
}

func UsernameTaken(context context.Context, database *sql.DB, username string) (bool, error) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UsernameTaken: Failed to close statement", "error", err)
		}
	}()
	var count int
...
func GetUserUuidByUsername(context context.Context, database *sql.DB, username string) uuid.UUID {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "GetUserUuidByUsername: Failed to close statement", "error", err)
		}
	}()
	var userUuid uuid.UUID
	err = stmt.QueryRow(username).Scan(&userUuid)
	assert.NoError(context, err, "Failed to get user")
	return userUuid
}

func GetUsername(context context.Context, database *sql.DB, userUuid uuid.UUID) string {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "GetUsername: Failed to close statement", "error", err)
		}
	}()
	var username string
...
func GetDiscordId(context context.Context, database *sql.DB, userUuid uuid.UUID) string {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "GetDiscordId: Failed to close statement", "error", err)
		}
	}()
	var discordId string
	err = stmt.QueryRow(userUuid).Scan(&discordId)
	assert.NoError(context, err, "Failed to get discord id")
	return discordId
}

func UpdateDiscordId(context context.Context, database *sql.DB, userUuid uuid.UUID, discordId string) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UpdateDiscordId: Failed to close statement", "error", err)
		}
	}()
	_, err = stmt.Exec(discordId, userUuid)
	assert.NoError(context, err, "Failed to update discord id")
}

// All crypto should happen before this since this just communicates with the DB
func ValidateLogin(context context.Context, database *sql.DB, username string, password string) bool {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "ValidateLogin: Failed to close statement", "error", err)
		}
	}()
	var dbPassword string
	err = stmt.QueryRow(username).Scan(&dbPassword)
	assert.NoError(context, err, "Failed to validate login")
...
func UpdatePassword(context context.Context, database *sql.DB, username string, newPassword string) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UpdatePassword: Failed to close statement", "error", err)
		}
	}()
	hashedPassword, err := bcrypt.GenerateFromPassword([]byte(newPassword), 14)
	assert.NoError(context, err, "Failed to generate password hash")
	_, err = stmt.Exec(string(hashedPassword), username)
	assert.NoError(context, err, "Failed to Update Password")
}
...
func RegisterSession(context context.Context, database *sql.DB, userUuid uuid.UUID, sessionToken string) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "RegisterSession: Failed to close statement", "error", err)
		}
	}()
	hasher := crypto.SHA256.New()
	hasher.Write([]byte(sessionToken))
	_, err = stmt.Exec(userUuid, hasher.Sum(nil))
	assert.NoError(context, err, "Failed to register session")
}

func UnRegisterSession(context context.Context, database *sql.DB, sessionToken string) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UnRegisterSession: Failed to close statement", "error", err)
		}
	}()
	hasher := crypto.SHA256.New()
	hasher.Write([]byte(sessionToken))
	_, err = stmt.Exec(hasher.Sum(nil))
	assert.NoError(context, err, "Failed to delete user session")
}

func GetUserBySessionToken(context context.Context, database *sql.DB, sessionToken string) uuid.UUID {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "GetUserBySessionToken: Failed to close statement", "error", err)
		}
	}()
	hasher := crypto.SHA256.New()
	hasher.Write([]byte(sessionToken))
	...
	assert.NoError(context, err, "Failed to get user")
	UpdateSessionExpiration(context, database, userUuid, sessionToken)
	return userUuid
}

func UserIsAdmin(context context.Context, database *sql.DB, userUuid uuid.UUID) bool {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UserIsAdmin: Failed to close statement", "error", err)
		}
	}()
	var isAdmin bool
...
func UpdateSessionExpiration(context context.Context, database *sql.DB, userUuid uuid.UUID, sessionToken string) {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "UpdateSessionExpiration: Failed to close statement", "error", err)
		}
	}()
	hasher := crypto.SHA256.New()
	hasher.Write([]byte(sessionToken))
	_, err = stmt.Exec(userUuid, hasher.Sum(nil))
	assert.NoError(context, err, "Failed to update session expiraton")
}

// Check if the session token is in the database and that it is not expired
func ValidateSessionToken(context context.Context, database *sql.DB, sessionToken string) bool {
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "ValidateSessionToken: Failed to close statement", "error", err)
		}
	}()
	hasher := crypto.SHA256.New()
	hasher.Write([]byte(sessionToken))
	...
	assert.NoError(context, err, "Failed to validate session")
...
func SearchUsers(context context.Context, database *sql.DB, searchString string, draftId int) ([]User, error) {
	query := `SELECT
                    Users.UserUuid,
                    Users.Username
                From Users
	...
	defer func() {
		if err := stmt.Close(); err != nil {
			log.Warn(context, "SearchUsers: Failed to close statement", "error", err)
		}
	}()
	...
	assert.NoError(context, err, "Failed to search users")
	defer func() {
		if err := userRows.Close(); err != nil {
			log.Warn(context, "SearchUsers: Failed to close rows", "error", err)
		}
	}()
