...
func (d *DraftDaemon) checkForDraftsToStart() error {
	...
	for _, draftId := range draftIds {
		...
		if err != nil {
			slog.Warn("Failed to load draft", "draftId", draftId, "error", err)
			continue
		}
...
func (d *DraftDaemon) checkForPicksToSkip() {
	for draftId, running := range d.runningDrafts {
		...
		//should skip them
		slog.Info("Checking if player wants to be skipped", "draftId", draftId, "currentPickPlayer", curPick.Player)
		shouldSkip := model.ShoudSkipPick(d.database, curPick.Player)
		if shouldSkip {
			slog.Info("Skipping player", "pickId", curPick.Id, "Player", curPick.Player)
		...
		slog.Info("Checking expiration time", "draftId", draftId, "currentPickPlayer", curPick.Player)
		now := time.Now()
		if curPick.ExpirationTime.Before(now) && !skipped {
			slog.Info("Pick expired", "pickId", curPick.Id, "expirationTime", curPick.ExpirationTime, "now", now)
			...
			slog.Info("Pick is not expired yet", "pickId", curPick.Id, "expirationTime", curPick.ExpirationTime, "now", now)
		}
	}
}
...
func (d *DraftDaemon) AddDraft(draftId int) error {
	...
	slog.Info("Added draft to daemon", "draftId", draftId)
	return nil
}
