...
func TestMatchListReq(t *testing.T) {
	tbaTok := getTbaTok(t)
	assert.NotEmpty(t, tbaTok, "TBA Token was not loaded correctly")
	handler := NewHandler(tbaTok, nil)
	matches, err := handler.MakeMatchListReq(t.Context(), "frc254", "2026casnv")
	require.NoError(t, err)
	assert.NotEmpty(t, matches, "No matches were found")
...
func TestEliminationAllianceRequestRetrySuccess(t *testing.T) {
	...
	assert.Len(t, alliances, 2, "Expected 2 alliances after retry")
	assert.Equal(t, "Alliance 1", alliances[0].Name)
	assert.Equal(t, "frc254", alliances[0].Picks[0])

	// Exponential backoff: 1s + 2s = 3s minimum
	assert.GreaterOrEqual(t, elapsed, 3*time.Second, "Expected at least 3s of backoff delay")
}
...
func TestEliminationAllianceRequestRetryExhausted(t *testing.T) {
	...
	assert.Empty(t, alliances, "Expected empty alliances after all retries exhausted")

	// Exponential backoff: 1s + 2s + 4s + 8s + 16s = 31s minimum
	assert.GreaterOrEqual(t, elapsed, 31*time.Second, "Expected at least 31s of backoff delay")
}
