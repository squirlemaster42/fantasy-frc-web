...
func main() {
	...
	if *populateTeams {
		log.InfoNoContext("Populating Teams")
		for _, event := range utils.Events() {
			log.DebugNoContext("Creating teams for event", "Event", event)
			for _, team := range tbaHandler.MakeTeamsAtEventRequest(event) {
				log.DebugNoContext("Checking if team is needed", "Team", team.Key, "Event", event)
				if model.GetTeam(database, team.Key) == nil {
					log.DebugNoContext("Creating team", "Team", team.Key, "Event", event)
