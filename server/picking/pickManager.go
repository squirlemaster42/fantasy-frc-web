...
func (p *PickManager) MakePick(pick model.Pick) (bool, error) {
	...
	if err == nil {
		...
		if err != nil {
			slog.Warn("Failed to get picks", "draftId", p.draftId, "error", err)
			return false, err
		}

		slog.Info("Checking if we should make another pick available", "numPicks", len(picks))
		if len(picks) < 64 {
			slog.Info("Making next pick available", "draftId", p.draftId)
			model.MakePickAvailable(p.database, nextPickPlayer.Id, time.Now(), utils.GetPickExpirationTime(time.Now()))
		} else {
