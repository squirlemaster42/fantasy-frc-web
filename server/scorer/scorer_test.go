...
func TestScoreMatches(t *testing.T) {
    ...
    match := tbaHandler.MakeMatchReq("2026casnv_qm1")
    ...
    match = tbaHandler.MakeMatchReq("2026casnv_qm20")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 0, scoredMatch.RedScore)
    assert.Equal(t, 3, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_qm38")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 4, scoredMatch.RedScore)
    assert.Equal(t, 1, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_qm52")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 0, scoredMatch.RedScore)
    assert.Equal(t, 5, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_qm74")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 0, scoredMatch.RedScore)
    assert.Equal(t, 4, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_qm24")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 1, scoredMatch.RedScore)
    assert.Equal(t, 5, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026mawne_qm40")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 6, scoredMatch.RedScore)
    assert.Equal(t, 1, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_sf4m1")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 15, scoredMatch.RedScore)
    assert.Equal(t, 0, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_sf6m1")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
    assert.Equal(t, 9, scoredMatch.RedScore)
    assert.Equal(t, 0, scoredMatch.BlueScore)

    match = tbaHandler.MakeMatchReq("2026casnv_f1m1")
    scoredMatch, _ = scorer.scoreMatch(match, true)
    assert.True(t, scoredMatch.Played)
