...
func (h *Handler) ConsumeTbaWebhook(c echo.Context) error {
	...
	setSpanAttrs(c, attribute.String("webhook.message_type", event.MessageType))
	log.Info(reqCtx, "Routing event", "Message Type", event.MessageType)
	// Detach async work from request lifecycle to preserve trace context
	// without the parent span ending when we return HTTP 200.
	asyncCtx := context.WithoutCancel(reqCtx)
	switch event.MessageType {
	case "upcoming_match":
		go h.HandleUpcomingMatchEvent(asyncCtx, event.MessageData)
	case "match_score":
		go h.HandleMatchScoreEvent(asyncCtx, event.MessageData)
	case "match_video":
		h.HandleMatchVideoEvent(reqCtx, event.MessageData)
	case "starting_comp_level":
		h.HandleCompLevelStartingEvent(reqCtx, event.MessageData)
	case "alliance_selection":
		go h.HandleAllianceSelectionEvent(asyncCtx, event.MessageData)
	case "awards_posted":
		h.HandleAwardsPostedEvent(reqCtx, event.MessageData)
	case "schedule_updated":
		h.HandleEventScheduleUpdatedEvent(reqCtx, event.MessageData)
	case "ping":
		h.HandlePingEvent(reqCtx, event.MessageData)
	case "broadcast":
		h.HandleBroadcastEvent(reqCtx, event.MessageData)
	default:
		log.Warn(reqCtx, "Unknown websocket event detected", "MessageType", event.MessageType, "Message", event.MessageData)
	}
