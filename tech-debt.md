...
| 7 | `server/scorer/scorer.go` | 304–369 | `scoringRunner` infinite loop never checks `ctx.Done()`, so it leaks on shutdown. | Add `ctx.Done()` checks and a graceful exit path. |
